...
export default function ShareModal({
  ...
  async function generateShareLink() {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || (language === 'de' ? 'Fehler beim Erstellen des Share-Links' : 'Error creating share link'));
        return;
      }
      ...
      setError(language === 'de' ? 'Verbindungsfehler' : 'Connection error');
    } finally {
      setLoading(false);
    }
  }
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                {itemType === 'directory' 
                  ? (language === 'de' ? `Ordner "${itemName}" teilen` : `Share folder "${itemName}"`)
                  : (language === 'de' ? `Datei "${itemName}" teilen` : `Share file "${itemName}"`)
                }
              </p>
            </div>
          </div>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  {language === 'de' ? 'Share-Link' : 'Share Link'}
                </label>
